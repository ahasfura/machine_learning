...
weights = {
    ...
    'wd2': tf.Variable(tf.random_normal([300, 100])), # fully connected, 7*7*64 inputs, 1024 outputs
    'out': tf.Variable(tf.random_normal([100, n_classes])) # 1024 inputs, 10 outputs (class prediction)
}
...
cost = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(pred, y) + l1 * tf.nn.l2_loss(weights['wd1']) + l2 * tf.nn.l2_loss(weights['wd2']) + l3 * tf.nn.l2_loss(weights['wd2']))
