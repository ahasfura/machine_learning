...
def conv_net(_X, _weights, _biases, _dropout):
    ...
    # Fully connected layer
    dense3 = tf.nn.relu(tf.add(tf.matmul(dense1, _weights['wd3']), _biases['bd3'])) # Relu activation

    # Output, class prediction
    out = tf.add(tf.matmul(dense3, _weights['out']), _biases['out'])
    return out
...
weights = {
    ...
    'wd2': tf.Variable(tf.random_normal([1024, 1024])), # fully connected, 7*7*64 inputs, 1024 outputs
    'wd3': tf.Variable(tf.random_normal([1024, 1024])), # fully connected, 7*7*64 inputs, 1024 outputs
