...
weights = {
    ...
    'wd2': tf.Variable(tf.random_normal([300, 150])), # fully connected, 7*7*64 inputs, 1024 outputs
    'out': tf.Variable(tf.random_normal([150, n_classes])) # 1024 inputs, 10 outputs (class prediction)
}

biases = {
    'bd1': tf.Variable(tf.random_normal([300])),
    'bd2': tf.Variable(tf.random_normal([150])),
