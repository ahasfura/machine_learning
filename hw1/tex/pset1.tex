...
Given training, test and validation data of the form, 
...
We ran a grid search optimization to find the best $\lambda$ and M for the ridge regression which minimized the sums of squares error. The minimum sum of squares error for the test data set occurred at M = 1 and $\lambda = 5$. The same procedure was followed on the validation set. Here the minimum sum of squares error was found to be at M=3 and  $\lambda = .1$. This means that the order of best fit polynomial is different for the test data and the validation data.  Taking the average of the errors from the test and validation sets leads to the optimal model parameters of $M=3$ $\lambda =.1$ but only by a small percentage as shown below
...
\begin{center}
  \begin{tabular}{ | c | c | c | c | c | }
    \hline
     ...
     .1 & 17.36 & 16.16 & \textcolor{red}{13.57}  \\ \hline
     .2 & 17.17 & \textcolor{red}{16.16} & 13.58   \\ \hline
    ...
    \hline
  \end{tabular}
  \caption{Average of the sum of squares error for the test and validation data sets. The red values indicate the minimum for each set order, M. Thus the optimal solution with this specified training set is thus a third order polynomial }
\end{center}
\end{table}

If we wanted to further validate that the correct model was indeed a third order polynomial, we could implement an exhaustive cross-validation and utilize each division of the data as a training set and test on the remaining sets. 
