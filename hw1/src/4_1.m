...
lambda=lambdas(i); 
    ...
    SSE0V(i)= .5*(norm(y0t-yval,1)+lambda*(w_ml0)'*w_ml0); 
    subplot(1,4,1)
    plot(xval, y0t, '*')
    hold on 
    %plot(X,Y, 'o')
    ...
    y1t= w_ml1(1)+w_ml1(2)*xval;  
      SSE1V(i)= .5*(norm(y1t-yval, 1)+lambda*(w_ml1)'*w_ml1);; 
 ...
 M=2; 
    ...
    y2t= w_ml2(1)+w_ml2(2)*xval+ w_ml2(3)*xval.^2; 
     SSE2V(i)= .5*(norm(y2t-yval, 1)+lambda*(w_ml2)'*w_ml2);; 
    ...
    y3t= w_ml3(1)+w_ml3(2)*xval+ w_ml3(3)*xval.^2+w_ml3(4)*xval.^3;
     SSE3V(i)= .5*(norm(y3t-yval, 1)+lambda*w_ml3'*w_ml3); 
    ...
    y4t= w_ml4(1)+w_ml4(2)*xval+ w_ml4(3)*xval.^2+w_ml4(4)*xval.^3+ w_ml4(5)*xval.^4;
     SSE4V(i)= .5*(norm(y4t-yval, 1)+lambda*(w_ml4)'*w_ml4); 
